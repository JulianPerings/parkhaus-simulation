...
public class PriceCalc {
  ...
  /**
   * calculates price for parking cars based on the time, they started parking and their parking
   * duration
   *
   ...
   */
  public double calcDayNightPrice(
      long beginInMillisec,
      long durationInMillisec) { // Time in milliseconds since unix; 3600000= hour; 86400000= day
    ...
    // Example: Lets say that 2 days in total were spent parking. We know that one day has 8 hours
    // of nighttime and
    // 16 hours of daytime. Thus, we know that 2*8 = 16 hours were spent parking at night and 2*16 =
    // 32 hours were spent parking at day.
    ...
    */
    ...
    parkedDay +=
        Math.max(overlap * ((currentTimeOfDay + durationInMillisec) % 86400000) - 3600000 * 6, 0);
    dayTime += parkedDay;
    nightTime += durationInMillisec - parkedDay;
    double pricePerMillisecond =
        Math.abs(
            priceInEuros
                / (nightTime + dayTime
                    + 1)); // +1 prevents dividing with 0 and has barely any effect
    double price =
        pricePerMillisecond * nightTime * nightPrice + pricePerMillisecond * dayTime * dayPrice;
  ...
  /**
   * beginInMillisec: starting time in milliseconds since unix start, durationInMillisec: duration
   * in milliseconds. Example: calcDayNightPrice(1651864726192, 1000)
   */
}
